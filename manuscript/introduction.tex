...
and Remark~\ref{rem:failure} records what the mixer generates when the hypotheses fail. The
Dicke-state plus Grover-mixer ansatz is exactly QWOA on the complete feasible graph, so the Grover mixer of Ref.~\cite{Bartschi2020} and the complete-graph
mixer of Ref.~\cite{Slate2021} are the same operator; the equivalence is noted in Ref.~\cite{Bridi2024} without the rescaling, which Proposition~\ref{prop:grover-complete-qwoa} supplies as $\theta=M\beta/2$. And the graph on which the ring $XY$ mixer walks has vertex degree $2b(x)$, twice the number of maximal cyclic blocks of ones in $x$, and is therefore not regular.

The second contribution is numerical. If every feasible mixer is a walk on a graph, the graph is a design parameter, and the Johnson and complete graphs
