...
{\color{red}\textbf{[NOVELTY AUDIT 2026 -- final positioning]} After including the 2026 literature, the paper should explicitly state what is \emph{not} new: (i) constraint-preserving $XY$ mixers; (ii) ring versus complete-$XY$ topology comparisons; (iii) the structural Johnson-graph interpretation of fixed-weight complete-$XY$ dynamics; (iv) graph-informed CTQW mixer design in general; and (v) DLA-based expressivity analysis as a methodology. The defensible novelty is the systematic Johnson-association-scheme interpolation/sweep and the resulting controlled evidence that increasing feasible-graph density is not monotonically beneficial, together with the proposed link among distance-class structure, spectrum, optimizer behavior, and circuit locality.}

{\color{red}\textbf{[LITERATURE UPDATE -- broader QWOA state of the art]} QWOA itself is also evolving beyond the variational setting: Bennett et al.~\cite{Bennett2026Nonvariational} report a nonvariational QWOA benchmark for weighted MaxCut. Cite this to avoid framing current QWOA research as exclusively variational and to delimit the present paper to variational feasible-subspace mixer design.}
